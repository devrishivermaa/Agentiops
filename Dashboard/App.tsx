...
import { LayoutDashboard, Settings, History, Activity, Wifi, WifiOff, Play } from 'lucide-react';
...
export default function App() {
  ...
  const handleUpload = (file: File) => {
    ...
    setTimeout(() => {
        // Start simulation
        simulatePipeline("pipe_" + Math.random().toString(36).substr(2, 9), (event) => {
  ...
  return (
    <div className="flex h-screen bg-background text-zinc-100 overflow-hidden font-sans">
      {/* Sidebar */}
      <aside className="w-16 md:w-64 flex flex-col border-r border-surfaceHighlight bg-surface shrink-0 z-20">
        <div className="h-16 flex items-center px-4 md:px-6 border-b border-surfaceHighlight">
          <Activity className="w-6 h-6 text-primary" />
          <span className="ml-3 font-bold text-lg hidden md:block tracking-tight">AgentOps</span>
        </div>

        <nav className="flex-1 p-4 space-y-2">
          <button 
            onClick={() => setActiveTab('upload')}
            className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg transition-colors ${activeTab === 'upload' ? 'bg-primary/10 text-primary' : 'text-zinc-400 hover:bg-zinc-800'}`}
          >
          ...
          <button 
             ...
             className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg transition-colors ${activeTab === 'dashboard' ? 'bg-primary/10 text-primary' : 'text-zinc-400 hover:bg-zinc-800'} ${pipeline.status === 'idle' ? 'opacity-50 cursor-not-allowed' : ''}`}
          >
            <Activity size={20} />
        ...
        <div className="p-4 border-t border-surfaceHighlight">
           <div className="flex items-center gap-3 px-3 py-2 text-zinc-500">
               {wsConnected ? <Wifi className="text-green-500" size={16} /> : <WifiOff className="text-red-500" size={16} />}
      ...
      <main className="flex-1 flex flex-col min-w-0 relative">
        ...
        <header className="h-16 border-b border-surfaceHighlight bg-background/50 backdrop-blur flex items-center justify-between px-6 z-10">
          <div>
            <h1 className="text-xl font-semibold">
              {activeTab === 'upload' ? 'Initialize Pipeline' : `Pipeline: ${pipeline.fileName || 'Active Session'}`}
            </h1>
            <p className="text-xs text-zinc-500">
                {pipeline.status === 'idle' ? 'Ready to process' : `Status: ${pipeline.status.toUpperCase()} • Step: ${pipeline.currentStep}`}
            </p>
          </div>
        ...
        <div className="flex-1 overflow-hidden flex flex-col md:flex-row relative">
            <div className="flex-1 relative overflow-hidden bg-[url('https://grainy-gradients.vercel.app/noise.svg')] bg-opacity-5">
                {activeTab === 'upload' && (
                    <FileUploader onUpload={handleUpload} isProcessing={pipeline.status === 'running'} />
                )}
                {activeTab === 'dashboard' && (
                    <AgentVisualization />
                )}
